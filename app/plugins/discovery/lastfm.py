"""Discover source: Last.fm's personalised "coming soon" releases page.
...
Results are cached in the DB for a while so opening the Discover page doesn't
...
def parse_releases(html):
    ...
    for li in soup.select("li.resource-list--release-list-item-wrap"):
        ...
        if ctx_p:
            ...
            # Keep the names themselves ("Similar to Quantic, Beanfield and
            # Jazzanova") so the feed can link each one to their page here.
            context_artists = [
                a.get_text(strip=True) for a in ctx_p.select("a") if a.get_text(strip=True)
            ]
...
class LastfmDiscovery(DiscoveryPlugin):
    """Last.fm 'coming soon' source. Needs the user's session cookie."""

    key = SOURCE
    label = "Last.fm"
    icon = "last-fm"
    description = "Your personalised \"coming soon\" releases from Last.fm."
    enabled_setting = "discover_lastfm_enabled"
    has_test = True
    config_fields = [
        {
            "key": "lastfm_cookie",
            "label": "Last.fm session cookie",
            "type": "textarea",
            "secret": True,
            "placeholder": "sessionid=...; csrftoken=...",
            "help": (
                "The Discover page scrapes your personalised Last.fm \"coming "
                "soon\" releases, which require a login. Paste your last.fm "
                "cookies here (at minimum sessionid=VALUE). Kept only in your "
                "local database."
            ),
        },
    ]
    ...
    def configured(self):
        ...
        return self.enabled() and bool((db.get_setting("lastfm_cookie") or "").strip())
