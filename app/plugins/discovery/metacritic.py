"""Discover source: Metacritic's upcoming album release calendar.
...
Results are cached in the DB (same TTL as the other Discover sources) so the
...
def _enrich(artist, album):
    """Return {image, genres} for a release. Best-effort, never raises.

    Cached twice over: in memory for the duration of a scrape, and in the
    database so a restart doesn't mean re-asking Last.fm and MusicBrainz about
    every release on the Discover page all over again.
    """
    ...
    with _enrich_lock:
        ...
        if hit and (time.time() - hit["at"]) < _ENRICH_TTL:
            return hit
    stored = db.get_json_cache(f"enrich:{key[0]}|{key[1]}", max_age=_ENRICH_TTL)
    if stored is not None:
        stored["at"] = time.time()
        with _enrich_lock:
            _enrich_cache[key] = stored
        return stored
    ...
    # Kept in the database as well: this cost up to three upstream calls.
    db.set_json_cache(f"enrich:{key[0]}|{key[1]}", result)
    with _enrich_lock:
...
class MetacriticDiscovery(DiscoveryPlugin):
    """Metacritic 'coming soon' album calendar. Public - just an on/off toggle."""

    key = SOURCE
    label = "Metacritic"
    description = "Upcoming album releases from Metacritic's public calendar."
    enabled_setting = "discover_metacritic_enabled"

    def fetch(self, force=False):
        return fetch_coming_soon(force=force)


register(MetacriticDiscovery())
